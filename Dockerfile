...
EXPOSE 8080
ENTRYPOINT ["java","-jar", "-javaagent:/usr/local/pinpoint-agent-2.5.2/pinpoint-bootstrap-2.5.2.jar", "-Dpinpoint.agentId=trendpick","-Dpinpoint.applicationName=adventcalendar","-Dpinpoint.config=/usr/local/pinpoint-agent-2.5.2/profiles/release/pinpoint.config", "-Dspring.profiles.active=prod","/app.jar"]
